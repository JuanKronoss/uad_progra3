...
/* */
CApp_Parcial3::CApp_Parcial3() :
	CApp(CGameWindow::DEFAULT_WINDOW_WIDTH, CGameWindow::DEFAULT_WINDOW_HEIGHT)
{
}

/* */
CApp_Parcial3::CApp_Parcial3(int window_width, int window_height, string jsonPath, string media_directory) :
	CApp(window_width, window_height), m_jsonGridFile(jsonPath), m_mediaDirectory(media_directory)
{
	cout << "Constructor: CApp_Parcial3(int window_width, int window_height)" << endl;
...
CApp_Parcial3::~CApp_Parcial3()
{
	cout << "Destructor: ~CApp_Parcial3()" << endl;
...
/* */
void CApp_Parcial3::initialize()
{
	// Initialize app-specific stuff here
	// ==================================
	//
	// ==================================
	m_myWorld.initializeWorld(getGameWindow()->getWidth(), getGameWindow()->getHeight());
	...
	m_myWorld.loadQuadtree();
}

/* */
void CApp_Parcial3::run()
{
...
/* */
void CApp_Parcial3::update(double deltaTime)
{
...
void CApp_Parcial3::render()
{
	...
	else // Otherwise, render app-specific stuff here...
	{
		...
		MathHelper::Matrix4 modelMatrixRotation = MathHelper::RotAroundX(0.0);
		modelMatrixRotation = MathHelper::RotAroundY(245.0);
		//modelMatrixRotation = MathHelper::RotAroundZ(0.0);

		MathHelper::Matrix4 modelMatrixTranslation = MathHelper::TranslationMatrix(0.0, 0.0, 0.0);
		MathHelper::Matrix4 modelMatrixScale = MathHelper::ScaleMatrix(1.0, 1.0, 1.0);
...
/* */
void CApp_Parcial3::onMouseMove(float deltaX, float deltaY)
{
...
/* */
void CApp_Parcial3::executeMenuAction()
{
	if (getMenu() != NULL)
	{
...
void CApp_Parcial3::onF2(int mods)
{
	setMenuActive(true);
...
void CApp_Parcial3::onArrowUp(int mods)
{
	float cameraFrontMovement = 0.1f;
	
	if (mods & KEY_MOD_CONTROL)
	{
		m_myWorld.m_camera->moveUp(cameraFrontMovement, mods);
	}
	else
	{
		m_myWorld.m_camera->moveForward(cameraFrontMovement, mods);
	}
}


void CApp_Parcial3::onArrowDown(int mods)
{
	float cameraFrontMovement = 0.1f;

	if (mods & KEY_MOD_CONTROL)
	{
		m_myWorld.m_camera->moveDown(cameraFrontMovement, mods);
	}
	else
	{
		m_myWorld.m_camera->moveBackward(cameraFrontMovement, mods);
	}
}


void CApp_Parcial3::onArrowLeft(int mods)
{
	float cameraLeftMovement = 0.1f;

	m_myWorld.m_camera->strafe(cameraLeftMovement, mods);
}


void CApp_Parcial3::onArrowRight(int mods)
{
	float cameraLeftMovement = -0.1f;
